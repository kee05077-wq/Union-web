"""
[방법 2: AIHub 공개 데이터] AIHub "수어 영상" 데이터셋에서 지화(지문자) 구간을 잘라
시퀀스 npy로 변환한다.
...
def iter_segments(annotation_json: dict):
    """라벨링 JSON 하나에서 (video_url, start_sec, end_sec, name) 튜플을 뽑아낸다.
    AIHub 설명에 따르면 형태소(morpheme) 단위로 URL/Duration/start/end/name 필드를 제공한다."""
...
def extract_segment_features(video_path: Path, start_sec: float, end_sec: float, detector: HolisticDetector):
    ...
    features = []
    for _ in range(max(end_frame - start_frame, 0)):
        ret, frame = cap.read()
        if not ret:
            break
...
def main():
    parser = argparse.ArgumentParser(description="AIHub 수어 영상 -> 지문자 시퀀스 npy 변환")
    ...
    for json_path in sorted(annotations_dir.glob("*.json")):
        ...
        for video_url, start_sec, end_sec, raw_name in iter_segments(annotation):
            ...
            frames = extract_segment_features(video_path, start_sec, end_sec, detector)
            if len(frames) < SEQ_LENGTH:
                print(f"  경고: 구간에서 인식된 프레임이 {len(frames)}개뿐이라 건너뜁니다.")
                continue
