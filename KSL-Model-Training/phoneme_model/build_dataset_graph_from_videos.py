"""[ST-GCN 실험용] record_dataset.py로 녹화한 영상을 그래프 시퀀스 npy로 변환한다.

build_dataset_from_videos.py(LSTM용, 55차원 벡터)와 같은 영상(dataset/videos/*.avi)을
그대로 재사용하되, 프레임당 손 관절을 (21, 2) 좌표 그래프 그대로 남겨
dataset/sequences_graph/<자모>_<n>.npy 에 저장한다 (shape: (N, SEQ_LENGTH, 21, 2)).
"""
import sys
...
from config import SEQ_LENGTH, VIDEO_DIR, SEQUENCE_GRAPH_DIR
from modules.holistic_module import HolisticDetector
from modules.utils import createDirectory, extract_phoneme_graph_keypoints
...
def process_video(video_path: Path, detector: HolisticDetector) -> np.ndarray:
    ...
    while True:
        ...
        feat = extract_phoneme_graph_keypoints(results.right_hand_landmarks)
        if feat is not None:
...
def main():
    createDirectory(str(SEQUENCE_GRAPH_DIR))
    ...
    for video_path in video_paths:
        ...
        out_path = SEQUENCE_GRAPH_DIR / f"{action}_{video_path.stem}.npy"
