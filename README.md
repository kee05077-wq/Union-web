...
출처: [AIHub 수어 영상](https://aihub.or.kr/aihubdata/data/view.do?currMenu=115&topMenu=100&aihubDataSe=data&dataSetSn=103)
(지화 1,000종 + 수어 단어 3,000종 + 수어 문장 2,000종, 형태소 단위 JSON 라벨링 포함)

AIHub는 로그인 및 이용 신청이 필요해 자동 다운로드는 지원하지 않습니다. 직접 내려받은 뒤:

```
python build_dataset_from_aihub.py --annotations-dir <라벨링 json 폴더> --videos-dir <mp4 폴더>
```

> **주의**: AIHub 라벨링 JSON의 정확한 필드명은 실제 다운로드본을 열어 확인이 필요합니다.
> 스크립트는 AIHub 공개 설명 기준(`URL`, `start`, `end`, `name`)으로 작성되어 있으니,
> 실제 파일과 다르면 각 스크립트의 `iter_segments()` 함수만 맞춰 고치면 됩니다.
> 라벨명이 우리 `ACTIONS`와 다르게 표기되어 있다면 `NAME_ALIASES` 딕셔너리에 매핑을 추가하세요.
